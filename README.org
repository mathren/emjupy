...
- [ ] implement "hide cell output" feature that collapses the "output"
  portion hiding the content and signaling in the output line that the
  output is hidden. Suggest toggling with =C-c h= maybe. This makes it
  easy to keep readable when inline figures or output is many lines.
- [ ] =C-c l= should not erase the bottom border
