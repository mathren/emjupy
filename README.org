...
Directories first, then notebooks, then other files, then the kernels
running on the server.  Within each group, most recently modified
first: the notebook worked on this morning is the one wanted, and
alphabetical order buries it among however many share its prefix.
Kernels have no modification time and stay sorted by name.
...
Development targets Emacs 30.1: what CI runs, and what most users are
on.  The package declares 29.1 as its minimum and is tested there too,
but 29 is not the version to develop in.  Eglot manages buffers
differently on the two, and a bug that sent every language server
request to a local process -- answering about the wrong machine's files
-- was invisible for weeks because 29 was where it was being tested.
...
- Aim: editing a cell same experience as editing a python file and we
  ...
  - [ ] =M-.= on a sibling module.  On Emacs 30 the WebSocket connected
    and Eglot left the shadow buffer unmanaged, so every request fell
    through to a language server on the local machine, which answers
    about the wrong files.  Fixed by activating Eglot's editing mode in
    that buffer explicitly.  Original report:
