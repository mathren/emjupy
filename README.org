...
- [ ] with =emjupy-remote-root= set, from the notebooklist view,
  pressing =d= sends to the root instead of the file on the line. Can we
  auto-set this variable to the project root? How does jupyter-lab
  access its root? Right now I'm polluting the remote home with files.
