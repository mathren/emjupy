...
- creating a new cell with =C-c C-b= now throws =[emjupy] Buffer and
  cells are out of step; not syncing.  Press C-c C-x C-l to redraw.=
  and shows 2 upper cell boundaries. This later led to me losing a
  cell when running.
- connecting to a second notebook on the same kernel gives =[emjupy] The kernel runs elsewhere, so completions describe THIS machine.  Set emjupy-shadow-host, or install jupyter-lsp on the server.=
