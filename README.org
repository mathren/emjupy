...
#+CAPTION: simple notebook rendering in [[https://github.com/mathren/emacs_mathren][my emacs]] with a slightly modified =wombat= dark theme. Screenshot may be out of date.
...
4. Run =emjupy-login= and follow the prompt. The token, password is =abc=
   provided in 1. The token is only asked for if the server actually
   needs one. emjupy adopts the kernel already running behind that
   port and shows you the notebook list; picking one drops you
   straight into that live REPL.

* Installation

Clone this repo, add to =load-path=, and load it:
...
Alternatively, use =install-package-file= giving it the tarball.

* Keybindings

** TODO  =emjupy-mode= (notebook buffers)
...
| remove =C-c C-e= | =emjupy-execute-cell-at-point=      | Same as =S-<return>=                                                 |
| =remove M-RET=      | =emjupy-execute-cell-and-goto-next= | Execute the cell and move to the next one                          |
| remove =C-c C-r=    | =emjupy-execute-cell-and-goto-next= | Same as =M-RET=                                                      |
...
| =C-c s=       | =emjupy-split-cell=                 | Split the cell at point in two                                     |
| =C-c m=       | =emjupy-merge-cell-above=           | Merge this cell into the one above (clears both outputs)           |
| =C-c C-t=     | =emjupy-cycle-cell-type=            | Toggle the cell between code and markdown                          |
| =C-c <up>=    | =emjupy-beginning-of-cell=          | Go to the start of this cell                                       |
| =C-c <down>=  | =emjupy-end-of-cell=                | Go to the end of this cell's source                                |
| =edit M-<up>= | =emjupy-move-cell-up=               | Move the cell one position earlier                                 |
| edit =M-<down>= | =emjupy-move-cell-down=             | Move the cell one position later                                   |
...
| remove =M-n=  | =emjupy-next-cell=                  | Same as =C-c C-n=                                                    |
| =C-c C-p=     | =emjupy-previous-cell=              | Move point to the previous cell                                    |
| =remove M-p=  | =emjupy-previous-cell=              | Same as =C-c C-p=                                                    |
| =C-x C-s=     | =emjupy-save-notebook=              | Write the notebook back to the server as nbformat v4               |
| =remove C-c C-s= | =emjupy-save-notebook=              | Same as =C-x C-s=                                                    |
...
** TODO emjupy design
describe shadow-buffer approach.
