...
- [ ] =eglot= still says it is missing on the remote machine. =[emjupy]
  The kernel runs elsewhere, so completions describe THIS machine. Set
  emjupy-shadow-host, or install jupyter-lsp on the server.
- [X] synthax highlight as typing doesn't always work (needs running
  the cell for proper fix often) user-error: Point is not in a code cell=. This is incorrect
...
- [X] integrate better with =tqdm=, which right now shows lines like: =0%|                                                                                           | 0/3 [00:00<?, ?it/s]=
