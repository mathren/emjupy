...
inserted, and a cell being deleted.  Every structural change now keeps it too: inserting, deleting, splitting
and merging cells all redraw only the cells involved.

Output and single-cell structural changes move only part of the buffer,
by a known amount, so the undo entries before the change are untouched
and those after it have their positions shifted to match.  A full
rebuild moves everything while undo recording is off, and undo entries
hold plain buffer positions rather than markers, so there is nothing
left to adjust and the history is dropped.
...
- [ ] when creating a new notebook, it should make a new cell and
  start cursor there. Now it's only background and I cannot start typing
- [ ] make the shadow file hidden on the remote, prepend =.emjupy_= to the name
- [ ] =C-c C-c= on markdown cell should trigger latex rendering
- [ ] make removing a cell an undo-able operation
- [ ] make emjupy-notebook-list clickable with mouse too
- [ ] when asking for port or url with (e.g., 8888) leaving empty
  should use 8888 as the default
** Hold
- [ ] upon reshowing output, the background color is scrambled (for
  output/warnings/errors/figures)
