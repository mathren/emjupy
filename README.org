...
=emjupy-shadow-host= does not say how to get there, no language server is
started on a shadow file at all.  One started there reads *this*
machine: the wrong interpreter, the wrong packages, and none of your own
modules.  Its answers would look plausible and be wrong, which is worse
than no answers, and it costs a server to produce them.
...
=jupyter-lsp= and a language server on the machine running Jupyter:

#+begin_src bash
pip install jupyter-lsp 'python-lsp-server[all]'
#+end_src

Check it is there with =GET /lsp/status= on your server, or just open a
notebook -- =emjupy-lsp-server= (default ="pylsp"=) names which of the
servers listed there to ask for.

Set =emjupy-lsp-enabled= to nil to fall back to the older Eglot
shadow-file arrangement, which needs no server-side support but pays
