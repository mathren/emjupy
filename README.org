...
3. Run =emjupy-login= and give it the port. That is the whole prompt:
...
emjupy is a multi-file Emacs package (see the Emacs Lisp manual, "Multi-file
Packages"). Build a tar and install it:
...
or from Emacs: =M-x package-install-file RET emjupy-0.1.0.tar RET=.

The only external dependency is =websocket= (1.15+); Emacs 29.1+ is required.
=emjupy-login= is autoloaded, so =M-x emjupy-login= works without any
=require=.
...
The fast, self-contained suite needs nothing but Emacs and =websocket=:
...
| Key         | Command                           | Effect                                           |
|-------------+-----------------------------------+--------------------------------------------------|
| =C-c C-z=     | =emjupy-connect-kernel-interactive= | pick/spawn a kernel *for this notebook*            |
| =C-c C-x C-r= | =emjupy-restart-kernel=             | restart *this* notebook's kernel only              |
| =C-c C-x C-c= | =emjupy-reconnect-kernel=           | re-open the socket to the SAME kernel            |
| =C-c C-x b=   | =emjupy-switch-notebook=            | jump between open notebooks (labelled by server) |
| =C-c C-x s=   | =emjupy-status=                     | list every notebook, its server, its kernel      |
| =C-c C-x l=   | =emjupy-login=                      | add another server                               |
...
Cells sit on the theme's normal background; the gaps between them show
a slightly contrasting canvas, so the notebook reads as a page of cells
rather than one undifferentiated wall of text.

Both colours are *derived from the active theme* rather than hard-coded:
=emjupy-canvas-color= defaults to ='auto=, which blends the default
background =emjupy-canvas-blend= (12%) of the way toward the default
foreground. That darkens a light theme and lightens a dark one, and
picks up the theme's tint -- a warm solarized background yields a warm
canvas, not a neutral gray. Themes switched mid-session are followed
automatically via =enable-theme-functions=.

To pin a specific colour instead:

#+begin_src elisp
(setq emjupy-canvas-color "#888888")   ;; any colour you like
(setq emjupy-canvas-color nil)         ;; or turn the effect off
#+end_src

=M-x emjupy-refresh-appearance= re-derives the colours and redraws open
notebooks. The relevant faces are =emjupy-cell= (inside the outline),
=emjupy-canvas= (outside it) and =emjupy-box-line= (the rule itself), all
customisable in the usual way.
