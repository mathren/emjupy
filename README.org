...
[[https://github.com/millejoh/emacs-ipython-notebook][unmaintained]], and the use of =polymode= makes it hard to use =eglot=.
Credit where it is due on undo: =ein= does support it within a cell, and
does so by maintaining a ledger mapping every =buffer-undo-list= entry to
a cell and shifting the positions inside those entries as cells change
length.  emjupy currently does less -- see
[[file:notes_undo.org][notes_undo.org]]. I need to share code with non-emacs users, so =ob-jupyter=
...
Undo works within a cell until something redraws the notebook -- running
a cell, output arriving from any cell, or adding, deleting, moving,
splitting or merging cells.  A redraw drops the undo history, so edits
made before it cannot be undone.  They are not lost: the text is still
in the buffer and in the saved notebook, only the record of how it got
there is gone.

The reason is that re-rendering erases the buffer and rebuilds it from
the cell structs.  Recording that is worse than not recording it -- an
early version did, and undoing one of those entries deleted every cell
after the one being edited -- so the rebuild is kept out of the history.
But undo entries hold plain buffer positions rather than markers, and
nothing adjusts them while recording is off, so entries from before a
rebuild may point anywhere.  Replaying one does not fail loudly; it
quietly changes the wrong text.  Dropping them loses undo, which is bad;
keeping them risks corrupting a notebook, which is worse.

This is a real gap against both =ein= and JupyterLab.
[[file:notes_undo.org][notes_undo.org]] measures what survives and what does not, explains how
each of those two avoids the problem, and sets out what closing it here
would involve.
