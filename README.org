...
- [ ] scrambling of the notebook still happens, check and fix all
  possible issues. cell boundaries piled at the top also exceed right
  boundary of buffer.
- [ ] change keybindings =C-c C-z= should stop the execution of the notebook
  and =C-c M-z= for kernel selection
- [ ] =C-c C-k= still jumps at the end of the buffer
...
- [ ] when doing =M-.= on a custom function in a notebook that is imported from
  a local =*.py= file, I would like that to go to the function in the
  library. Right now I get =No definition found for: LSP identifier at point=
