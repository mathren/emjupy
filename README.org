...
- [ ] implement =C-c C-l= for clean current cell output (from anywhere
  in the input or output area of the cell) and =C-c C-u C-l= for clean
  all cells outputs. Check if compatible with MELPA guidelines first.
