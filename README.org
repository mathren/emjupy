...
=ein= was my working tool for more than a decade through my PhD and
postdocs, but it is unmaintained, and the use of polymode breaks undo
and makes it hard to use =eglot=. I need to share code with non-emacs
users, so =ob-jupyter= isn't an option, and =emacs=jupyter= doesn't seem
...
one cell at a time would see seven unrelated files and resolve nothing
across them.
