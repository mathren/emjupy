...
One consequence of the read-only regions is worth knowing: a kill whose
region runs off the end of a cell is refused, but the text still reaches
the kill ring, because Emacs copies before it deletes.  You get an error
and a working =C-y=.
...
cell.  Run a long cell, move down to edit another while it works, let
its output land: the edits are still there and still undoable.

Output arriving redraws only the cell it
belongs to, rather than rebuilding the notebook; and the undo entries
that the redraw moved have their positions shifted to match, rather than
being thrown away.

What still clears the history is a *structural* change -- adding,
deleting, moving, splitting or merging cells -- which rebuilds the whole
buffer.  Edits made before such a change cannot be undone afterwards.
They are not lost: the text is in the buffer and in the saved notebook,
only the record of how it got there is gone.

[[file:notes_undo.org][notes_undo.org]] has the measurements, how =ein= and JupyterLab handle the
same problem, and what closing the remaining gap would involve.
...
- [ ] make =undo= possible after structural changes (maybe using markers
  just for the destructive actions?)
- [ ] avoid full rebuild for split/merge/erase/new cell: these actions
  affect at most 2 cells at a time, why can't we just redraw these and
  leave the =undo= alone?
