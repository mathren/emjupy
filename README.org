...
4. Follow the prompt in the minibuffer area. The token/password is =abc=
   provided in 1.,only asked for if the server actually needs one.
   emjupy adopts the kernel already running behind that port and shows
   you the notebook list; picking one drops you straight into that
   live REPL.
...
| =C-c s=       | =emjupy-split-cell=                 | Split the cell at point in two (clears both outputs)               |
| =C-c m=       | =emjupy-merge-cell-above=           | Merge this cell into the one above (clears both outputs)           |
| =C-c w=       | =emjupy-copy-cell=                  | Copy the cell, without its output                                  |
| =C-c y=       | =emjupy-yank-cell=                  | Insert the copied cell below, unrun                                |
...
defined in cell 3 is used in cell 7. A language server handed one cell
at a time would see seven unrelated files and resolve nothing across
them.

So emjupy keeps a hidden *shadow buffer*: one ordinary Python file
holding every code cell concatenated, each preceded by a marker comment
naming its cell.  Eglot attaches to that file and sees exactly what it
...
*** Why a hidden buffer rather than =polymode=
...
of formulae does not re-run LaTeX, and switching theme regenerates them
rather than serving back black glyphs on a dark background.  emjupy uses a minimal preamble (amsmath and
amssymb) rather than org's full default package list, which pulls in
packages a small TeX install does not have.
...
- [ ] add to *.el files =;; Assisted-by: Claude:claude-opus-5 and other free-tier LLMs=
- [ ]
