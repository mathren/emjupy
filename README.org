...
- [ ] although eldoc works perfectly, doing =M-.= on a function says "No
  definition found" or tries local files.

- [ ] upon reshowing output, background is ok, but text get python
  highlighted (including removing background)

- [ ] add =C-v= for create cell above. This also should not break undo.
