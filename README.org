...
- [ ] although eldoc works perfectly, doing =M-.= on a function
  created a local path and file (the path would be correct on remote,
  instead of opening the existing remote file that eldoc previews.
  Upon trying it on a module function it created a file again in the
  right path but on the wrong machine
  =emjupy-lsp-diagnose= shows
  #+BEGIN_SRC
notebook        test.ipynb
server          localhost:9999
server root     /home/mrenzo/Runs/LMXRB/scripts
kernel cwd      /home/mrenzo/Runs/LMXRB/scripts
lsp enabled     yes
lsp url         ws://localhost:9999/lsp/ws/pylsp?token=<redacted>
shadow file     /home/mrenzo/Runs/LMXRB/scripts/.emjupy_localhost_9999__test.ipynb.py
language server over the Jupyter WebSocket
connection      running
back-off        none
socket failure  none recorded

If no language server is attached, ask the Jupyter server directly:
  curl -s 'http://localhost:9999/lsp/status?token=TOKEN'
  404 no jupyter-lsp   403 wrong token   sessions {} no language server
#+END_SRC

- [ ] undo fails on executing cell and the cursor jumps to the
bottom. It shouldn't fail, and if it does the cursor should not move
and maybe use the bell? But the important thing is preventing the
failure
