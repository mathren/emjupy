...
- Aim: editing a cell same experience as editing a python file and we
  ...
  - [ ] dynamic expansion with =M-/= are not found (even when the
    expansion is in the notebook or library file)
  - [ ] =M-;= says it doesn't know which comment sign to use
- [ ] =C-c C-t=, also =TAB= moves the view, they should not. Both put the
  cursor at the bottom of the buffer on the last line visible. These
  jumps make it unusable.
