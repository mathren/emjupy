...
- [ ] use =*= or =\ | / -= loop for "executing" status of a cell between
  running it and output showing up for long cells.
- [ ] suppress "can't find python offset" message in every cell
- [ ] upon large output of a notebook (large figure), whole notebook
  can get scrambled. shadow buffer seems ok. Why? Maybe add function
  `emjupy-re-render`?
- [ ] implement =M-<pgup>=/=<pgdown>= for go to the beginning/end of
  previous cell and =M-<up>=/=<down>= for go to the beginning/end of
  next cell
